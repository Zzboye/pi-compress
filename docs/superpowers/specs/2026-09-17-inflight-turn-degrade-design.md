...
| 3 | 片段渲染层级 | L1 起步（刚发生的事信息最全），瀑布照走：L1→L2→L3→L4 常规降级；**L5 豁免**（见裁定 7） |
| 4 | 切分边界 | 完整 toolCall→toolResult 对，不拆对 |
| 5 | turn 结束收敛 | 整 turn 摘要落盘 → 队列中片段跑完 → 片段墓碑化；整 turn 从 L1 重新走瀑布 |
| 6 | 执行方式 | 独立分支，先不合并 main |
| 7 | 片段 L5 豁免 | turn 结束前片段最多降到 L4：L5 的合并收益对片段为零（L4 每片段仅 ~2 行），而其「拒绝召回」代价在长任务进行中是实害（模型可能需要取回早期片段原文）；引擎 L5 组成员筛选跳过进行中 turn 的片段 |
...
| 情况 | 行为 |
|---|---|
| 片段降级到 L5 边界 | **豁免**：引擎 L5 组成员筛选跳过进行中 turn 的片段（按 key 判定：条目 key ≠ turn.startEntryId 且属于当前进行中 turn），保持 L4 |
...
2. **摘要与瀑布**：片段入队→落盘→渲染为 L1 行；瀑布对片段与普通 turn 同待遇，但 L5 组成员筛选跳过进行中片段（豁免用例：长任务多片段 + L5 触发 → 片段保持 L4、普通旧 turn 正常合并）；失败保持原文
