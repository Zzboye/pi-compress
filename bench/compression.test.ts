/**
 * 基准 ②：压缩效果——原生上下文 token vs 装配后 token（与 findWindowTurns 同口径 countTokens（CJK 感知））。
...
function tokensOf(entries: Array<{ message: any }>): number {
  let t = 0;
  for (const e of entries) t += countTokens(e.message);
  return t;
}
...
describe("compression: token 压缩比 @ 递增会话规模（keepRecentTokens=20000）", () => {
  it("30 / 100 / 300 / 1000 turns", () => {
    ...
    for (const n of [30, 100, 300, 1000]) {
      ...
      let ledgerTokens = 0;
      for (const d of cache.values()) ledgerTokens += countTokens(renderActionLedger([d]));
      rows.push({
        turns: n,
